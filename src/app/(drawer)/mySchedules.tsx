...
export default function MySchedules() {
  ...
  //const [listNextDates, setListNextDates] = useState<ScheduledSchema[]>([]);
  ...
  useEffect(() => {
    ...
    if (dataScheduled) {
      ...
      //setListNextDates(filterLoja.filter(item => new Date(`${item.appointmentDate}T00:00`) > new Date()))
    }
    setisLoading(false)
  }, [dataScheduled])
  ...
  if (!listDates.length) {
    return (
      <VStack flex={1} backgroundColor="#f9f9f9">
        <DrawerHeader title="Meus agendamentos" />
        <Center flex={1}>
